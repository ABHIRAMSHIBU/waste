...
void remove_epsilon_all_productions(struct production *p,int production_count,char *first_result[100]){
	for(int i=0;i<production_count;i++){
		int eps_pos_first = search_string(first_result[i],'e');
		if(eps_pos_first==-1){
			continue;
		}
		int production_epislon_present = -1;
		for(int j=0;j<p[i].n_rhs;j++){
			if(search_string(p[i].rhs[j],'e')!=-1){
				production_epislon_present = 1;
				break;
			}
		}
		if(production_epislon_present==-1){
			remove_character(first_result[i], 'e');
		}
	}
}

void find_first_second_parse(struct production *p,int production_count,char *first_result[100]){
	for(int i=0;i<production_count;i++){
		if(search_string(first_result[i],'e')!=-1 || 1){
			for(int j=0;j<production_count;j++){
				for(int k=0;k<p[j].n_rhs;k++){
					int pos = search_string(p[j].rhs[k],p[i].lhs);
					if(pos!=-1){
						int check = 1;
						for(int l=pos-1;l>=0;l--){
							int prod_pos = search_production(p, production_count, p[j].rhs[k][l]);
							if(prod_pos==-1){
								check=-1;
								break;
							}
							else if(search_string(first_result[prod_pos],'e')==-1){
								check=-1;
								break;
							}
						...
						if(check==1){
							check_concatenate(first_result[j],first_result[i]);
						}
					}
				}
