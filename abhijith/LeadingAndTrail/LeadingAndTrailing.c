...
void parse(optable t,struct stack s,char *input){
	int ip = 0;
	while(s.a[s.top]!='$' && input[ip]!='$'){
		printf("input[%d]=%d\n",ip,input[ip]);
		printf("Stack state is ");
		display_char_array(s.a);
		printf("\n");
		if(getPrecidence(t,s.a[s.top],input[ip])=='<'||getPrecidence(t,s.a[s.top],input[ip])=='='){
			printf("<\n");
			printf("pusing to stack");
			push(&s,input[ip]);
			printf("Stack state is ");
			display_char_array(s.a);
			printf("\n");
		...
		else if(getPrecidence(t, s.a[s.top], input[ip])=='>'){
			printf("popping from stack ");
			display_char_array(s.a);
			printf("\n");
			char popped = pop(&s);
			while(getPrecidence(t, s.a[s.top],popped)!='<'){
